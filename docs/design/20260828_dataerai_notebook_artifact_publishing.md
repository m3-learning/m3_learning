...
The helper reuses source datasets by a stable, exact asset title; captures full
...
manifests as model assets. All uploads go through the active `NotebookSession`,
allowing the SDK trace to stamp the notebook run identity and create
`records_telemetry` relationships from the execution log.
...
| Publish model bundle | saved fitter paths and generated manifest | `test_model_checkpoint_loss_and_manifest_are_model_assets_with_lineage` |
